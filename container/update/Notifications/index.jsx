...
export const Notification = () => {
  ...
  const [createOneNotification, { data }] = useMutation(CREATE_NOTIFICATION, {
    ...
    onError: (error) => setAlertBox({ message: error.message }),
  ...
  return (
    ...
    <Text> {dataForm?.message}</Text>
      <Row margin='20px 0'>
        ...
        <RippleButton disabled={dataForm?.message?.length <= 0} widthButton='50%' margin='0' padding='0' onClick={() => sendNotification()}>
          Subir
        </RippleButton>
      </Row>
    </Column>
